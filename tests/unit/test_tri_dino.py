...
def test_lora_routing_changes_output_when_b_nonzero() -> None:
    ...
    with torch.no_grad():
        ...
        baseline_depth = tower(depth, "depth")

    # Inflate the depth LoRA-B for the very first wrapper.
    first_wrapper = tower.lora_modules["depth"][0]
    with torch.no_grad():
        first_wrapper.lora_B["depth"].add_(0.5)

    with torch.no_grad():
        new_rgb = tower(rgb, "rgb")
        new_depth = tower(depth, "depth")

    # rgb path uses the rgb LoRA paths -> unchanged
    assert torch.allclose(baseline_rgb, new_rgb), "rgb output should be invariant"
    # depth path uses the depth LoRA paths -> changed
    assert not torch.allclose(baseline_depth, new_depth), "depth output should differ"
