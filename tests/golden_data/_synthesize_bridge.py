...
"""Synthesize a tiny BridgeData V2 stand-in fixture.

Bridge V2 runs at 5 Hz (action_chunk = 4 in the canonical schema), uses the
Bridge WidowX embodiment (``bridge_widowx``), and has both ``head_rgb`` and
``wrist_rgb`` cameras.
"""
...
FPS_NATIVE = 5  # Bridge is 5 Hz native
FPS_FEATURES = 5
CAMERAS = ("head_rgb", "wrist_rgb")
...
def synthesize_tiny_bridge(out_root: Path, seed: int = 0xB121) -> Path:
    """Materialize a tiny Bridge V2 fixture."""
    ...
    try:
        ...
        raise RuntimeError(
            "pyarrow is required to materialize the tiny_bridge fixture"
        ) from e
    ...
    # Bridge's native fps == features fps so 1 feature frame per native frame.
    n_feat_frames = max(N_FRAMES, 4)
    for cam in CAMERAS:
...
if __name__ == "__main__":  # pragma: no cover
    ...
    p.add_argument("--out", type=Path, default=Path(__file__).parent / "tiny_bridge")
    args = p.parse_args()
    out = synthesize_tiny_bridge(args.out)
    print(f"wrote tiny_bridge fixture to {out}")
