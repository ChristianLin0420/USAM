...
"""Synthesize a tiny OXE-AugE stand-in fixture (generic embodiment).

OXE-AugE aggregates many sub-sources under a single canonicalization
manifest. We only need *one* mock sub-source to exercise the multi-source
mixture sampler; the embodiment is the catch-all ``oxe_auge_generic``.
"""
...
def _make_action_native(rng: np.random.Generator, T: int) -> np.ndarray:
    ...
    a[:, 0:3] = rng.uniform(-0.4, 0.4, size=(T, 3)).astype(np.float32)
    a[:, 3:6] = rng.uniform(-0.8, 0.8, size=(T, 3)).astype(np.float32)
...
def synthesize_tiny_oxe_auge(out_root: Path, seed: int = 0x0AE6) -> Path:
    """Materialize a tiny OXE-AugE fixture."""
    ...
    try:
        ...
        raise RuntimeError(
            "pyarrow is required to materialize the tiny_oxe_auge fixture"
        ) from e
...
if __name__ == "__main__":  # pragma: no cover
    ...
    p.add_argument("--out", type=Path, default=Path(__file__).parent / "tiny_oxe_auge")
    args = p.parse_args()
    out = synthesize_tiny_oxe_auge(args.out)
    print(f"wrote tiny_oxe_auge fixture to {out}")
